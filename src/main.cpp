...
void init_draw(int playtime)
{
  ...
  mvwprintw(win2, 2, 1, "B: 0 / 0");
  mvwprintw(win2, 3, 1, "+: 0");
  mvwprintw(win2, 4, 1, "-: 0");
  mvwprintw(win2, 5, 1, "G: 0");
  mvwprintw(win2, 7, 1, "PlayTime");
  mvwprintw(win2, 8, 1, "%d",playtime);
...
void update()
{
  ...
  while(running)
  {
    ...
    //head가 아이템과 닿으면 itemvec에서 item을 삭제
    for(int i = 0; i<Itemvec.size(); i++)
    {
      if(y == Itemvec[i].pos.x && x == Itemvec[i].pos.y)
      {
  ...
  for(int i = 0; i < HEIGHT; i++)
  {
    for(int j = 0; j < WIDTH; j++)
    {
      ...
      else if(map[i][j] == 0)                   // 게임화면의 Field 색상 적용
      {
        ...
        mvwprintw(win1, i, j*2, "%d%d", map[i][j], map[i][j]);
        wattroff(win1, COLOR_PAIR(3));
        //item이 생성되면 포지션의 색상을 변경
        if(itemIs)
        {
          Itemvec[Itemvec.size()-1].setItemPos();
          map[Itemvec[Itemvec.size()-1].pos.x][Itemvec[Itemvec.size()-1].pos.y] = 3;
        }
